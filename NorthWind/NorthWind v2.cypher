// -- NorthWind Graph Data Model Import v2 --//
...
MATCH (f:OrderStatus_FulfilleD {Status: "Fulfilled"}), (n:Order)-[:HAS_SHIPMENT_INFO]->(s:ShipInfo)
WHERE s.ShippedDate IS NOT NULL
CREATE (f)-[:IS_FULFILLED {FulfillDate: datetime() }]->(n); 

MATCH (o:OrderStatus_OpeN {Status: "Open"}), (n:Order)-[:HAS_SHIPMENT_INFO]->(s:ShipInfo)
...
MATCH (r:Regions)-[:HAS_TERRITORY]->(t:Territory)-[:HAS_EMPLOYEE]->(e:Employee)<-[:SOLD_BY]-(o:Order)-[details:HAS_PRODUCT]->(p:Product)
...
MATCH (cat:CategorY)-[:HAS_PRODUCT]->(p:Product)<- [details:HAS_PRODUCT]-(o:Order)
...
MATCH (c:Customer)<-[:HAS_CUSTOMER]-(o:Order)-[:HAS_PRODUCT]->(p:Product)
<-[:HAS_PRODUCT]-(o2:Order)-[:HAS_PRODUCT]->(p2:Product)<-[:HAS_PRODUCT]-(:CategorY)-[:HAS_PRODUCT]->(p)
...
CREATE (o:Order {OrderID:"N10000", OrderDate:date(), RequireDate:date()+duration("P7D")})<-[:IS_OPEN]-(p)
...
MATCH (a)<-[:HAS_CUSTOMER_ADDRESS]-()<-[:HAS_CUSTOMER]-(o:Order {OrderID:"N10000"})<-[r]-(p:OrderStatus_OpeN {Status: "Open"}), (s:Shipper {ShipperID:"1"}), (f:OrderStatus_FulfilleD {Status: "Fulfilled"})
CREATE (i:ShipInfo {ShippmentID:randomUUID(), ShippedDate:date()})
CREATE (o)-[:HAS_SHIPMENT_INFO]->(i)
CREATE (i)-[:HAS_SHIPPER]->(s)
CREATE (f)-[:IS_FULFILLED {FulfillDate: datetime()}]->(o)
CREATE (i)-[:HAS_SHIPMENT_ADDRESS]->(a)
DELETE r
RETURN o, i, s;
...
CREATE (cat)-[:HAS_PRODUCT]->(p)
CREATE (p)-[:HAS_SUPPLIER]->(sup)
CREATE (sup)-[:SUPPLIES]->(p)
CREATE (avail)-[:IS_AVAILABLE]->(p)
RETURN p, i;
...
MATCH (c)<-[:HAS_CUSTOMER]-(o:Order)-[:HAS_PRODUCT]->(p:Product)
