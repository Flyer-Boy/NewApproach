...
// The following nodes and relatinships are an extension of the Model   
...
// We add the existing Suppilers to the ApprovedSupplierS state
MATCH (a:ApprovedSupplierS {Name:"ApprovedSupplierS"}), (s:Supplier)
CREATE (a)-[:IS_SUPPLIER_APPROVED_STATE]->(s);


// We now create the aditional employees that will be pat of the Supplier, PO and RFQ vetting workflows 
...
// Place a random Customer order with 1 to 11 products. We will execute this a few times in order to have some Open Orders to fulfill later
...
// Let's check what Product have Stock Levels below the Restock level so we can place new POs to their respective suppliers 
// We will set a Minimum order to the 
...
// Let's check the open customers orders by Product 
...
// Create the PO Domain Collections Nodes
...
// As we move the PO's though the Vetting we will connect the PO State to the PO's
// We will use the following Ontology for this:
// For Approved PO's: (PO)<-[:IS_APPROVED_PO_STATE {Date:datetime()}]-(:ApprovedPoS {Name:"ApprovedPoS"}) - PO's rejected during the vetting workflow 
// For Rejected PO's: (PO)<-[:IS_REJECTED_PO_STATE {Date:datetime()}]-(:RejectedPoS {Name:"RejectedPoS"}) - PO's approved during the vetting workflow 
// For Approved PO's: (PO)<-[:IS_SUBMITTED_PO_STATE {Date:datetime()}]-(:SubmittedPoS {Name:"SubmittedPoS"}) - Approved PO's submited to the Supplier so they can send RFQ 
// For Approved PO's: (PO)<-[:IS_CLOSED_PO_STATE {Date:datetime()}]-(:ClosedPoS {Name:"ClosedPoS"}) - PO's Closed (RFQ approved and products delivered or all RFQs rejected or PO canceled, etc. )
...
// The Procurment Assistant will create PO's to Suppliers for Products that have low Inventory Level  
  // We select a random Employee with the role "Procurment Assistant"
...
WHERE i.UnitsInStock <= r.StockThreshold
     // Group the low-stock products and calculations per supplier
WITH e, n, s, COLLECT({
    Product: p, 
...
// Check the PO's created: 
MATCH n=(p:PurchaseOrder)-[]->() RETURN n ;

//Let's get the PO costs per product and total cost per PO
...
// Find the sum for each specific purchase order
...
//Summary of New (pending approval) PO's by Total Cost
...
// Let's have the L1 approver approve all the POs'
// As we are selecting the PO's that are bellow the L1 Approval Limit, and moving them to Approved State
...
// Level 1 is now ready to vet the New (pending approval) POs that have cost beyond her Level, so he/she cannot fully Approve them, 
// however he/she will still vet them so the next Approval level can vet
...
// The following command is not supported by Cypher, BUT it would be very helpful if it did as one Cypher comamnd will sufice for both cases - 
// I am commenting it and putting it here for enhancment resquest to the Graph Database provider 
...
// Now we move to Level 2 vetting - Simikat to Level 1 but we will check if L1 has already Approved
...
//Finally Level 3 vetting - Same as L2 
...
// You can add as many vetting levels you want following the same patern. Simple, isn't it? 

// Next up in the workflow is the "Buyer" who will review the Approved PO's (vet them if he will) and submit them to the respective Suppliers so they can send theyr RFQ's fr each PO 
// Before we start submitting PO's to the Suppliers we must create the proper Graph schemas for the Suppliers to recive ans submit RFQ's and for the PO's to accept/reject RFQ's 

// Supplires muts have a collection to hold the New PO's so they can query "What new/open PO's I have to submit RFQ's?" - here you might probably say:  "Easy, check all the Approved/Submitted PO's from procurment that have me as the supplier" 
// However, we don't want a query from the Supplier fronend to have access to all the Approved/Submitted PO's from Procurmemt, but only to a subset contolled by Procurment containing their PO's. 
// This subset that the Supplier has access is this collecton I am refreing to. 
// Procurment will then "connect" the Submitted PO's in this Supplier Collection where the Supplire can see only the ones that matters to them.
// There is a lot more on access rights that I have not covered in my code comments and in my articles, but trust me, everything in this model takes access rights into consideration. 
// Our original models/graph where all these concepts came from was DRM to the Node level. This was a huge overhead at the time but framed our work and models.
// I will write about this in one article at some point, for now, lets get our model working
