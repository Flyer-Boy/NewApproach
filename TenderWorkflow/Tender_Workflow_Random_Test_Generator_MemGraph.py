...
# ---------------------------------------------------------------------------
# Cypher call counting
# ---------------------------------------------------------------------------
#
# CountingDriver transparently wraps the real neo4j Driver so every
# session.run() call anywhere in this script increments a shared counter --
# without needing to thread a counter argument through every function that
# already calls driver.session()/session.run(). Every function in this file
# keeps working unmodified; they just receive a CountingDriver instead of a
# raw Driver, and it behaves identically from their point of view.

class _CountingSession:
    def __init__(self, session, counting_driver: "CountingDriver"):
        self._session = session
        self._counting_driver = counting_driver

    def run(self, *args, **kwargs):
        self._counting_driver.call_count += 1
...
class CountingDriver:
    """Drop-in wrapper around a neo4j Driver that counts every Cypher
    statement executed via session.run(), across every session opened."""

    def __init__(self, driver: Driver):
        self._driver = driver
        self.call_count = 0

    def session(self, *args, **kwargs) -> _CountingSession:
        return _CountingSession(self._driver.session(*args, **kwargs), self)
    ...
    with driver.session() as session:
        session.run(
            ...
            CREATE (t1:Tender {
                        ...
                        EndBidingDate: datetime() + duration({day: $days_to_bid}),
                        Budget: $budget
            ...
            session.run(
                ...
                CREATE (vb)-[:HAS_ACTIVE_BID {Date: datetime()}]->
                       (b:Bid {
                            ...
                            CompletionDate: datetime() + duration({day: $completion_days}),
                            Price: $price,
...
def award_bid(driver: Driver, tender_code: str, winning_bid_code: str, all_bid_codes: list[str]) -> None:
    ...
    with driver.session() as session:
        ...
        session.run(
            ...
            CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m:Message {Text:"Congratulations! Your Bid "  + $bid_code +  " for Tender " + $code  + " has been awarded to you! We look forward for to starting the Project as soon as possible."})-[:SENT_BY]->(e)
            """,
            code=tender_code, bid_code=winning_bid_code,
        ) 
        ...
        session.run(
            ...
            CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m:Message {Text:"We regret to inform that your Bid for Tender "  + $code  + " has not been awarded to you. We thank you for your participation and look forward to working with you in the future."})-[:SENT_BY]->(e)
            """,
            code=tender_code, 
        )
...
def main() -> None:
    ...
    uri = os.environ.get("MEMG_URI", "bolt://localhost:7687")
    user = os.environ.get("MEMG_USERNAME", "")
    password = os.environ.get("MEMG_PASSWORD", "")
    if not password:
        raise SystemExit("Set the MEMG_PASSWORD environment variable before running.")


    driver = CountingDriver(GraphDatabase.driver(uri, auth=(user, password)))
    try:
        ...
        # Baseline taken *after* the reference-data lookup above, so the
        # per-cycle and simulation totals below reflect only the randomized
        # Tender/Bid workflow itself -- not the one-time setup queries.
        simulation_start_calls = driver.call_count
        simulation_start_time = time.perf_counter()

        for i in range(1, args.tenders + 1):
            print(f"--- Tender lifecycle {i}/{args.tenders} ---")
            cycle_start_calls = driver.call_count
        ...
        total_elapsed = time.perf_counter() - simulation_start_time
        avg_elapsed = total_elapsed / args.tenders if args.tenders else 0.0
        print("=== Simulation summary ===")
        print(f"Tender cycles run:            {args.tenders}")
        print(f"Total Cypher commands issued: {total_calls}")
        print(f"Total elapsed time:           {total_elapsed:.3f}s")
        print(f"Average time per cycle:       {avg_elapsed:.3f}s")
    finally:
        driver.close()
