...
// In v7.1, I removed the common [:HAS] relationship type entirely and adopted
...
// GRAPH TYPE brings a new level of rigor to the model, allowing to think of this approach I am utilizing in this POC in a more pratyical way, 
// closer to what a real-world ontology Line of Business (LOB) Apllicatin would be. 
...
// The current state of the art in LOB application development is still largely dominated by RDBMS-based systems, which are often complex, rigid, and costly to maintain.
// By bulding LOB applications on Graph databases, we can achieve a more natural and efficient representation of workflows, relationships, and context,
// allowing for more intuitive and efficient development processes. 
// Having a LOB application built on a Graph database, with a built-in and locked Ontology, where you can add additional context layers and vector information directly on the data source allows LLM's to 
// understand, reason and interact on the source data (No ETL needed).
...
//      (There’s a deeper explanation behind this, involving the way the Graph is stored (how Node properties and its adjacency table is stored), and the implications for indexing, query optimization, and performance,
//       as well as Node digital rights management (DRM) considerations, but I won’t go down that rabbit hole here. However they are very relevant ones.)
...
// Before we start, let's clear all Nodes and relationships - Constraints will be set by a separate sctipt using Cypher v25 - GRAPH TYPE     
...
// Create 19 Employees and link them to the Employee Directory Node (this will likely come from the Company's Active Directory and use SSO to authenticate the users with proper Employee ID).
...
CREATE  (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e1:Employee {Name:"Albert", ID:"S12345678N", Phone:"443344331", Email:"Albert@email.com" , Photo:"/src/assets/images/People/4139948.png" })<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq),
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e2:Employee {Name:"Brandon", ID:"S84765968N", Phone:"944453321", Email:"Brandon@email.com" , Photo:"/src/assets/images/People/4139993.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq), 
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e3:Employee {Name:"Charles", ID:"S324538736N", Phone:"997766544", Email:"Charles@email.com" , Photo:"/src/assets/images/People/4140050.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq), 
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e4:Employee {Name:"Donald", ID:"S67828333N",  Phone:"944455541", Email:"Donald@email.com" , Photo:"/src/assets/images/People/4140057.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}] -(rq),  
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e5:Employee {Name:"Eduard", ID:"S98376432N",  Phone:"955445551", Email:"Eduard@email.com" , Photo:"/src/assets/images/People/4140037.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq), 
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e6:Employee {Name:"Fred", ID:"S25984432N",  Phone:"909876665", Email:"Fred@email.com" , Photo:"/src/assets/images/People/4139981.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq),  
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e7:Employee {Name:"Gilbert", ID:"S093983333N", Phone:"55446773", Email:"Gilbert@email.com" , Photo:"/src/assets/images/People/4140055.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq), 
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e8:Employee {Name:"Harold", ID:"S763663638N",  Phone:"484747443", Email:"Harold@email.com" , Photo:"/src/assets/images/People/4140063.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq), 
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e9:Employee {Name:"Ingrid", ID:"S18487473N",  Phone:"998474731", Email:"Ingrid@email.com" , Photo:"/src/assets/images/People/4140060.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq), 
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e10:Employee {Name:"Juliet", ID:"S17477328N",Phone:"998474744", Email:"Juliet@email.com" , Photo:"/src/assets/images/People/4140078.png" })<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq),
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e11:Employee {Name:"Keneth", ID:"T84735968N",  Phone:"9494453321", Email:"Keneth@email.com" , Photo:"/src/assets/images/People/6997675.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq),
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e12:Employee {Name:"Marlon", ID:"T324518736N", Phone:"9909136544", Email:"Marlon@email.com" , Photo:"/src/assets/images/People/4140050.png" })<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq),
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e13:Employee {Name:"Nelson", ID:"T67028333N", Phone:"9449851541", Email:"Nelson@email.com" , Photo:"/src/assets/images/People/4140057.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(rq);

MATCH (ed:EmployeeDirectorY {Name:"EmployeeDirectorY"}), (va:RolE {Name:"VendorApprover"})
CREATE  (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e14:Employee {Name:"Robert", ID:"T98846432N", Phone:"9551062551", Email:"Robert@email.com" , Photo:"/src/assets/images/People/4140037.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(va),
        (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e15:Employee {Name:"Mary", ID:"T25980932N", Phone:"909870092", Email:"Mary@email.com" , Photo:"/src/assets/images/People/4140076.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(va);

MATCH   (ed:EmployeeDirectorY {Name:"EmployeeDirectorY"}), (l1:RolE {Name:"Level1Approver"})
CREATE  (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e16:Employee {Name:"Gloria", ID:"T093982633N", Phone:"559831373", Email:"Gloria@email.com" , Photo:"/src/assets/images/People/4140047.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(l1);

MATCH   (ed:EmployeeDirectorY {Name:"EmployeeDirectorY"}), (l2:RolE {Name:"Level2Approver"})       
CREATE  (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e17:Employee {Name:"Sara", ID:"T763663208N", Phone:"4849810343", Email:"Sara@email.com" , Photo:"/src/assets/images/People/4143768.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(l2);

MATCH   (ed:EmployeeDirectorY {Name:"EmployeeDirectorY"}), (l3:RolE {Name:"Level3Approver"})
CREATE  (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e18:Employee {Name:"Christine", ID:"T14987473N", Phone:"9998344731", Email:"Christine@email.com" , Photo:"/src/assets/images/People/4140071.png"})<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(l3);

MATCH   (ed:EmployeeDirectorY {Name:"EmployeeDirectorY"}), (p:RolE {Name:"Publisher"})
CREATE  (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(e19:Employee {Name:"Cloe", ID:"T17488328N", Phone:"998195044", Email:"Cloe@email.com" , Photo:"/src/assets/images/People/4140069.png" })<-[:IS_ACTIVE_ROLE {StartDate:datetime()}]-(p);
...
CREATE (ed)-[:HAS_ACTIVE_EMPLOYEE {StartDate:datetime()}]->(:Employee {Name:"System", ID:"SYSTEM", Email:"system@email.com"});
...
// AI Agent Query:
MATCH (:PendingVendorS {Name:"PendingVendorS"})-[:IS_VENDOR_PENDING_STATE]->(v)-[:HAS_VENDOR_DOCS]->()-[:HAS_VENDOR_DOCUMENT]->(d:VendorDoc {DocName:"VendorRFIDoc"}) RETURN v.VendorCode, v.ShortName, d.URL;
...
MATCH (:PendingVendorS {Name:"PendingVendorS"})-[:IS_VENDOR_PENDING_STATE]->(v)
WITH v 
CREATE (v)-[:HAS_AI_AGENT_VENDOR_ASSESSMENT {Date:datetime()}]->(:AIVendorAssessment {Rating:rand(), 
...
MATCH (:PendingVendorS {Name:"PendingVendorS"})-[:IS_VENDOR_PENDING_STATE]->(v:Vendor)-[:HAS_AI_AGENT_VENDOR_ASSESSMENT]->(ai:AIVendorAssessment), (v)-[:HAS_VENDOR_DOCS]->()-[:HAS_VENDOR_DOCUMENT]->(d:VendorDoc {DocName:"VendorRFIDoc"})
...
MATCH (pv:PendingVendorS {Name:"PendingVendorS"})-[r1:IS_VENDOR_PENDING_STATE]->(v:Vendor {ShortName:"Vendor 1"}), (l1:Employee {Name:"Robert"})<-[:IS_ACTIVE_ROLE]-(:RolE {Name:"VendorApprover"}), (av:ApprovedVendorS {Name:"ApprovedVendorS"})
CREATE (v)-[:HAS_L1_VENDOR_APPROVAL {Date:datetime(), Comment:"This vendor is approved"}]->(l1),
       (av)-[:IS_VENDOR_APPROVED_STATE]->(v),
...
MATCH (pv:PendingVendorS {Name:"PendingVendorS"})-[r1:IS_VENDOR_PENDING_STATE]->(v:Vendor {ShortName:"Vendor 3"}), (l1:Employee {Name:"Mary"})<-[:IS_ACTIVE_ROLE]-(:RolE {Name:"VendorApprover"}), (av:ApprovedVendorS {Name:"ApprovedVendorS"})
CREATE (v)-[:HAS_L1_VENDOR_APPROVAL {Date:datetime(), Comment:"This vendor is approved"}]->(l1),
       (av)-[:IS_VENDOR_APPROVED_STATE]->(v),
       (v)-[:HAS_ACCEPTED_INVITATIONS]->(:AcceptedInvitationS {Name:"AcceptedInvitationS"}),
       (v)-[:HAS_ACTIVE_BIDS]->(:ActiveBidS {Name:"ActiveBidS"}),
...
MATCH (pv:PendingVendorS {Name:"PendingVendorS"})-[r1:IS_VENDOR_PENDING_STATE]->(v:Vendor {ShortName:"Vendor 4"}), (l1:Employee {Name:"Robert"})<-[:IS_ACTIVE_ROLE]-(:RolE {Name:"VendorApprover"}), (av:ApprovedVendorS {Name:"ApprovedVendorS"})
CREATE (v)-[:HAS_L1_VENDOR_APPROVAL {Date:datetime(), Comment:"This vendor is approved"}]->(l1),
       (av)-[:IS_VENDOR_APPROVED_STATE]->(v),
       (v)-[:HAS_ACCEPTED_INVITATIONS]->(:AcceptedInvitationS {Name:"AcceptedInvitationS"}),
       (v)-[:HAS_ACTIVE_BIDS]->(:ActiveBidS {Name:"ActiveBidS"}),
...
MATCH (pv:PendingVendorS {Name:"PendingVendorS"})-[r1:IS_VENDOR_PENDING_STATE]->(v:Vendor {ShortName:"Vendor 7"}), (l1:Employee {Name:"Mary"})<-[:IS_ACTIVE_ROLE]-(:RolE {Name:"VendorApprover"}), (av:ApprovedVendorS {Name:"ApprovedVendorS"})
CREATE (v)-[:HAS_L1_VENDOR_APPROVAL {Date:datetime(), Comment:"This vendor is approved"}]->(l1),
       (av)-[:IS_VENDOR_APPROVED_STATE]->(v),
       (v)-[:HAS_ACCEPTED_INVITATIONS]->(:AcceptedInvitationS {Name:"AcceptedInvitationS"}),
       (v)-[:HAS_ACTIVE_BIDS]->(:ActiveBidS {Name:"ActiveBidS"}),
...
MATCH (pv:PendingVendorS {Name:"PendingVendorS"})-[r1:IS_VENDOR_PENDING_STATE]->(v:Vendor {ShortName:"Vendor 8"}), (l1:Employee {Name:"Robert"})<-[:IS_ACTIVE_ROLE]-(:RolE {Name:"VendorApprover"}), (av:ApprovedVendorS {Name:"ApprovedVendorS"})
CREATE (v)-[:HAS_L1_VENDOR_APPROVAL {Date:datetime(), Comment:"This vendor is approved"}]->(l1),
       (av)-[:IS_VENDOR_APPROVED_STATE]->(v),
       (v)-[:HAS_ACCEPTED_INVITATIONS]->(:AcceptedInvitationS {Name:"AcceptedInvitationS"}),
       (v)-[:HAS_ACTIVE_BIDS]->(:ActiveBidS {Name:"ActiveBidS"}),
...
MATCH (pv:PendingVendorS {Name:"PendingVendorS"})-[r1:IS_VENDOR_PENDING_STATE]->(v:Vendor {ShortName:"Vendor 10"}), (l1:Employee {Name:"Mary"})<-[:IS_ACTIVE_ROLE]-(:RolE {Name:"VendorApprover"}), (av:ApprovedVendorS {Name:"ApprovedVendorS"})
CREATE (v)-[:HAS_L1_VENDOR_APPROVAL {Date:datetime(), Comment:"This vendor is approved"}]->(l1),
       (av)-[:IS_VENDOR_APPROVED_STATE]->(v),
       (v)-[:HAS_ACCEPTED_INVITATIONS]->(:AcceptedInvitationS {Name:"AcceptedInvitationS"}),
       (v)-[:HAS_ACTIVE_BIDS]->(:ActiveBidS {Name:"ActiveBidS"}),
...
MATCH (pv:PendingVendorS {Name:"PendingVendorS"})-[r1:IS_VENDOR_PENDING_STATE]->(v:Vendor {ShortName:"Vendor 2"}), (l1:Employee {Name:"Robert"})<-[:IS_ACTIVE_ROLE]-(:RolE {Name:"VendorApprover"}), (rv:RejectedVendorS {Name:"RejectedVendorS"}) 
CREATE (v)-[:HAS_L1_VENDOR_REJECTION {Date:datetime(), Comment:"This vendor is rejected"}]->(l1),
       (rv)-[:IS_VENDOR_REJECTED_STATE]->(v)
       DELETE r1;
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m1:Message {Text:"Hi, I have a question about the tender. Can you please clarify the budget?"})-[:SENT_BY]->(l1);

MATCH (e:Employee)<-[:HAS_REQUESTER]-(t:Tender {Title:"Tender 1"})-[:HAS_TENDER_CHAT]->(c) 
CREATE  (c)-[:HAS_MESSAGE { Date:datetime()}]->(m2:Message {Text:"Sure, the budget is 400000 in order to include the international deployment as discussed on the meeting of March 10th. Is there anything else you need?"})-[:SENT_BY]->(e);

MATCH (t:Tender {Title:"Tender 1"})-[:HAS_TENDER_CHAT]->(c), (l1:Employee {Name:"Gloria"})
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m3:Message {Text:"No, that will be all. Thank you!"})-[:SENT_BY]->(l1);           
...
CREATE (t)-[:HAS_L1_TENDER_APPROVAL {Date:datetime(), Comment:"This tender is approved"}]->(l1);
...
CREATE (t)-[:HAS_L2_TENDER_APPROVAL {Date:datetime(), Comment:"This tender is approved"}]->(l2);
...
CREATE (t)-[:HAS_L3_TENDER_APPROVAL {Date:datetime(), Comment:"This tender is approved"}]->(l3),
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m1:Message {Text:"Hi, I have a question about the tender. Can you please clarify the criteria for selecting the 2 vendors?"})-[:SENT_BY]->(l2);
...
CREATE  (c)-[:HAS_MESSAGE { Date:datetime()}]->(m2:Message {Text:"Sure, the criteria are based on the vendors experience and the price they offered in previous tenders. Is there anything else you need?"})-[:SENT_BY]->(e);

// Let's have the Approver 2 (Sara) Reply.
MATCH (t:Tender {Title:"Tender 2"})-[:HAS_TENDER_CHAT]->(c), (l2:Employee {Name:"Sara"}) 
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m3:Message {Text:"No, that will be all. Thank you!"})-[:SENT_BY]->(l2);
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m1:Message {Text:"Hi Ingrid, I have a question about the tender. Reading thrugh the documentation, it seems that this Tender is not complient with our Tender norms as per Schedule 11?"})-[:SENT_BY]->(l1);

MATCH (e:Employee)<-[:HAS_REQUESTER]-(t:Tender {Title:"Tender 9"})-[:HAS_TENDER_CHAT]->(c) 
CREATE  (c)-[:HAS_MESSAGE { Date:datetime()}]->(m2:Message {Text:"Opps, my bad! You are right. We overlooked this part. Sorry"})-[:SENT_BY]->(e);

MATCH (t:Tender {Title:"Tender 9"})-[:HAS_TENDER_CHAT]->(c), (l1:Employee {Name:"Gloria"})
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m3:Message {Text:"No Proble. I will reject it. So if you are willing to move forward, fix the compliance parts and resubmit it. Thank you!"})-[:SENT_BY]->(l1);          
...
CREATE (t)-[:HAS_L1_TENDER_REJECTION {Date:datetime(), Comment:"This tender is rejected"}]->(l1),
...
CREATE (t)-[:HAS_PUBLISHER_TENDER_APPROVAL {Date:datetime(), Comment:"This tender is approved for Publishing"}]->(p),
...
CREATE (c)-[:HAS_MESSAGE {Date:datetime()}]->(m:Message {Text:"You have been Invited to participate on a tender (Tender 1)."})-[:SENT_BY]->(e);
...
MATCH (v:Vendor {ShortName:"Vendor 7"})<-[:IS_VENDOR_APPROVED_STATE]-(:ApprovedVendorS)
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m:Message {Text:"You have been Invited to participate on a tender (Tender 3)."})-[:SENT_BY]->(e);
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m:Message {Text:"You have been Invited to participate on a tender (Tender 4)."})-[:SENT_BY]->(e);
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m:Message {Text:"Tender 4 is now officially Closed for bidding. We will communicate the results as soon as we have an awarded bid. Thanks for participating."})-[:SENT_BY]->(e);
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m1:Message {Text:"Hi, I have a question about the bid. Can you please justify the price and delivery dates?"})-[:SENT_BY]->(e);

MATCH (vc:VendorContact)<-[:HAS_CONTACT]-(v:Vendor)<-[:HAS_VENDOR]-(b:Bid {BidCode:"B79c69d9843ff"})-[:HAS_BID_CHAT]->(c)
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m2:Message {Text:"Hi Albert. Sure, the price is 80000 given the complexity of the project. As we will need to bring extra resources and might have to hire a few new resources, the timeframe might be a bit longer than usual. Is there anything else you need?"})-[:SENT_BY]->(vc);

MATCH (b:Bid {BidCode:"B79c69d9843ff"})-[:HAS_BID_CHAT]->(c), (e:Employee {Name:"Albert"})  
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m3:Message {Text:"Thanks, Herbert. No, that will be all. Thank you!"})-[:SENT_BY]->(e);
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m4:Message {Text:"Hi Herbert, this is Gloria from the Approval team and I would like to know how committed you are for delivering on the proposed time?"})-[:SENT_BY]->(l1);

// The Vendor 7 contact person will reply to Gloria's question
MATCH (vc:VendorContact)<-[:HAS_CONTACT]-(v:Vendor)<-[:HAS_VENDOR]-(b:Bid {BidCode:"B79c69d9843ff"})-[:HAS_BID_CHAT]->(c)
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m5:Message {Text:"Hi Gloria, we are fully committed to delivering on the proposed time. We have the resources and the expertise to deliver on time. Is there anything else you need?"})-[:SENT_BY]->(vc);

// Gloria thanks the Vendor 7 contact person for the reply
MATCH (b:Bid {BidCode:"B79c69d9843ff"})-[:HAS_BID_CHAT]->(c), (l1:Employee {Name:"Gloria"})  
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m6:Message {Text:"Thanks Herbert, that will be all from my side. Thanks!"})-[:SENT_BY]->(l1);
...
WITH b 
CREATE (b)-[:HAS_AI_AGENT_BID_ASSESSMENT {Date:datetime()}]->(:AIBidAssessment {Rating:rand(), 
AssessmentSummary:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi ornare id justo ac varius. Aliquam in dui consequat, pulvinar lorem quis, rhoncus ipsum. Aenean mi dolor, dapibus ac urna eget, condimentum facilisis urna. Sed vulputate fermentum odio, ut mattis magna rhoncus sit amet. Morbi vestibulum tortor et diam placerat, et tristique mauris porta. Sed tellus diam, aliquam ac ullamcorper malesuada, iaculis nec sem. Mauris felis risus, fringilla in pellentesque a, porta eget arcu. Proin id leo eu tortor dapibus pellentesque at eget justo. Nunc quis euismod erat. Nullam non interdum nibh. Sed ipsum erat, feugiat quis mauris et, tempor laoreet nulla. Duis tincidunt dolor a tortor accumsan, vitae eleifend est efficitur. Suspendisse efficitur, justo non malesuada feugiat, leo enim tincidunt tortor, non tempor nunc magna at purus.", 
Advantages:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer blandit auctor sem, ut pharetra erat mollis id. Vestibulum eu mi venenatis, lacinia orci nec, laoreet diam. Maecenas auctor ac ante eu vestibulum. Sed vitae placerat ex, sed pharetra ex. Donec pharetra mi lacus. Etiam et dapibus erat, a mattis diam. Pellentesque rhoncus tellus ac sem pellentesque laoreet. Sed accumsan maximus odio sed molestie.", 
Disadvantages:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla bibendum elementum tristique. Fusce neque nisl, fermentum eget tellus elementum, semper consectetur dui. Morbi eros arcu, vehicula ac magna ut, lacinia ornare felis. Integer tincidunt mi vel dolor convallis pharetra. Cras viverra congue nisi et imperdiet. In posuere vehicula commodo. Ut viverra sapien arcu, vel maximus leo feugiat non."});
...
CREATE  (b)-[:HAS_L3_BID_APPROVAL {Date:datetime(), Comment:"This bid is approved"}]->(l3),
        (aw)-[:IS_AWARDED_TENDER_STATE]->(t),
        (t)-[:HAS_AWARDED_VENDOR {Date:datetime()}]->(v), // This relationship is redundant (optional), as the Bid already has a relationship with the Vendor, but it is kept for clarity and future reference.
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m:Message {Text:"Congratulations! The Bid for Tender 1 has been awarded to you! We look forward for to starting the Project as soon as possible."})-[:SENT_BY]->(e);
...
MATCH (e:Employee {Name:"System"}), (t:Tender {Title:"Tender 1"})-[:HAS_TENDER_BIDS]->(:TenderBidS)-[:HAS_TENDER_BID]->(b)-[:HAS_VENDOR]->(v)-[:HAS_VENDOR_CHAT]->(c:ConversatioN) WHERE NOT (t)-[:HAS_AWARDED_BID]->(b)
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m:Message {Text:"The Bid for Tender 1 has not been awarded to you. We thank you for your participation and look forward to working with you in the future."})-[:SENT_BY]->(e);
...
MATCH (t:Tender {Title:"Tender 1"})-[:HAS_TENDER_BIDS]->(:TenderBidS)-[:HAS_TENDER_BID]->(b)<-[r1:HAS_ACTIVE_BID]-(ab)<-[:HAS_ACTIVE_BIDS]-(v)-[:HAS_PAST_BIDS]->(pb) WHERE NOT (t)-[:HAS_AWARDED_BID]->(b)
WITH t , v , r1 , b, pb
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m:Message {Text:"You have been Invited to participate on a tender (Tender 10)."})-[:SENT_BY]->(e);
...
MATCH (v:Vendor {ShortName:"Vendor 10"})<-[:IS_VENDOR_APPROVED_STATE]-(:ApprovedVendorS)
        CREATE (iv)-[:HAS_INVITATION {Date:datetime()}]->(v);
...
CREATE (c)-[:HAS_MESSAGE { Date:datetime()}]->(m:Message {Text:"You have been Invited to participate on a tender (Tender 12)."})-[:SENT_BY]->(e);
